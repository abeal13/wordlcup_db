...
echo "$($PSQL "SELECT winner.name AS team_name FROM games INNER JOIN teams AS winner ON games.winner_id = winner.team_id WHERE round = 'Eighth-Final' AND year = '2014' UNION ALL SELECT opponent.name AS team_name FROM games INNER JOIN teams AS opponent ON games.opponent_id = opponent.team_id WHERE round = 'Eighth-Final' AND year = '2014' ORDER BY team_name ASC")"
